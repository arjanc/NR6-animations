...
function resizeGrip() {
  ...
  scale = Math.min(wrapper.getBoundingClientRect().width / originalGripSize.width, wrapper.getBoundingClientRect().height / originalGripSize.height);
...
function initScrollMagic() {
  ...
  controller = new ScrollMagic.Controller(); // Build scenes.
...
window.onload = function () {
  ...
  for (var _i = 0; _i < gripImages.length; _i++) {
    // create new image object
    // so we can add a listener to it.
    var img = new Image();
    img.src = gripImages[_i].src;
    img.addEventListener('load', function () {
      ...
      if (imagesLoaded === gripImages.length) {
        ...
        console.log('originalGripSize: ', originalGripSize); // now we initialize the rest...
